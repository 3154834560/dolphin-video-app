...
public class CommentListener implements View.OnClickListener {
    ...
    public void onClick(View v) {
        ...
        commentListAddListener(commentList, linear);
    ...
    private void commentListAddListener(ListView commentList, LinearLayout linear) {
        int[] layoutIds = new int[]{R.id.comment_user_head_portrait, R.id.comment_user_nick,
                R.id.comment_date_time, R.id.comment_content, R.id.delete_comment};
        String[] listItemNames = new String[]{"commentList"};
        List<Map<String, CommentListView>> listItems = getCommentData(context, videoId);
        CommentListViewAdapter listViewAdapter = new CommentListViewAdapter(context, linear, listItems, R.layout.comment_list_view, listItemNames, layoutIds, videoId);
    ...
    public static List<Map<String, CommentListView>> getCommentData(Context context, String videoId) {
        ...
        for (CommentInput commentInput : Objects.requireNonNull(StringPool.COMMENT_INPUT_MAP.getOrDefault(videoId, new ArrayList<>()))) {
            CommentListView commentListView = commentInput.toCommentListView(AuthorInfoActivity.class);
            Map<String, CommentListView> listViewMap = new HashMap<>();
            listViewMap.put("commentList", commentListView);
